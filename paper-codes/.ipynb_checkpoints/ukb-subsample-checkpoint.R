...
ref_wegene <- paste0("/home/share/UKB/ld_ref_2k/height_ukb_ldpred_ref_2000_noMHC_2M")
...
bim <- bim[match(sumstats_wegene$SNP,bim$V2),]

idx_snp <- match(sumstats_wegene$SNP,test_geno$snps)
...
idx_flip <- which(bim$V5!=sumstats_wegene$ALLELE1)     # find flipped allele
sumstats_wegene$BETA[idx_flip] <- -sumstats_wegene$BETA[idx_flip]    # flip beta
pval_wegene <- data.frame(rsid=sumstats_wegene$SNP,pval=sumstats_wegene$P_BOLT_LMM_INF)
...
for(l in 1:length(thr_all)){
  if(all(pval_wegene$pval>thr_all[l])){   # if no significant SNPs passing thr
    ...
    idx_clp <- match(clp_wegene$rsid,sumstats_wegene$SNP)
