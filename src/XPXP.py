...
if __name__ == '__main__':
    ...
    for i,sumsf in enumerate(sumst_files):
        ...
        logger.info("Sumstats {} shape: {}x{}".format(sumst_names[i], sumstats_dict[sumst_names[i]].shape[0],\
            sumstats_dict[sumst_names[i]].shape[1])) 
    ...
    for i in range(ngroup):
        ...
        beta_estimate_mu[idx_i,:], beta_estimate_muxpxp[idx_i,:] = ComputePMXPXP(i, sumst_names, \
                sumst_ref1, ns, p, L1, L2, Sigma_beta, Sigma_e, zs_b, logger, return_LDpredinf=args.return_LDpredinf)

    beta_estimate_mu[:,:len(sumst_ref1)] = (beta_estimate_mu[:,:len(sumst_ref1)]/np.sqrt(p))/X1_sd.reshape(-1,1)
    beta_estimate_mu[:,len(sumst_ref1):] = (beta_estimate_mu[:,len(sumst_ref1):]/np.sqrt(p))/X2_sd.reshape(-1,1)
    beta_estimate_muxpxp[:,:len(sumst_ref1)] = (beta_estimate_muxpxp[:,:len(sumst_ref1)]/np.sqrt(p))/X1_sd.reshape(-1,1)
    beta_estimate_muxpxp[:,len(sumst_ref1):] = (beta_estimate_muxpxp[:,len(sumst_ref1):]/np.sqrt(p))/X2_sd.reshape(-1,1)
